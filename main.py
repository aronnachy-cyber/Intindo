...
# --- ⚙️ Bot Setup ---
# তোর দেওয়া সেই টোকেনটি এখানে সেট করা হয়েছে
...
# কিক এবং ব্যান কমান্ড যা রোল হায়ারার্কি চেক করে কাজ করবে
@bot.command()
...
# নতুন কেউ জয়েন করলে ড্যাশবোর্ড থেকে সেট করা মেসেজ ও র‍্যান্ডম GIF পাঠাবে
@bot.event
async def on_member_join(member):
    if config["welcome_system"] == "ON" and config["welcome_channel"]:
        channel = bot.get_channel(int(config["welcome_channel"]))
        if channel:
            # ড্যাশবোর্ড থেকে GIF লিঙ্কগুলো নিয়ে লিস্ট বানানো এবং র‍্যান্ডমলি একটা সিলেক্ট করা
            gif_list = [g.strip() for g in config["welcome_gifs"].split(",")]
            selected_gif = random.choice(gif_list)
            
            # মেসেজ থেকে {user} ট্যাগ রিপ্লেস করা
            welcome_text = config["welcome_msg"].replace("{user}", member.mention)
            
            embed = discord.Embed(description=welcome_text, color=discord.Color.blue(), timestamp=datetime.datetime.utcnow())
            embed.set_image(url=selected_gif)
            embed.set_footer(text=f"Member Count: {len(member.guild.members)}")
            await channel.send(content=member.mention, embed=embed)

# --- 🤖 3. Automod & Social Notifier ---
@bot.event
async def on_message(message):
    if message.author.bot: return

    # Anti-Link Filter: যদি ড্যাশবোর্ড থেকে অন থাকে
    if config["block_links"] == "ON" and ("http" in message.content or "www" in message.content):
        await message.delete()
        return await message.channel.send(f"🚫 {message.author.mention}, Links are not allowed here!", delete_after=5)

    # Social Notifier (YouTube & Facebook Link Detection)
...
# --- 📝 4. Logging System ---
# মেসেজ ডিলিট হলে লগ চ্যানেলে অটো রিপোর্ট যাবে
@bot.event
async def on_message_delete(message):
    if config["log_channel"]:
        log_chan = bot.get_channel(int(config["log_channel"]))
        if log_chan:
            embed = discord.Embed(title="🗑️ Message Deleted", color=0xffa500, timestamp=datetime.datetime.utcnow())
...
if __name__ == "__main__":
    # ১. ড্যাশবোর্ডকে আলাদা থ্রেডে রান করা
